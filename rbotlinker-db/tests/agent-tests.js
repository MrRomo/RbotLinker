...
let usernameArgs = {
  where: { username: 'rbotlinker', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.rbotlinker))
...
test.serial('Agent#findByUsername', async t => {
  let agents = await db.Agent.findByUsername('rbotlinker')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.rbotlinker.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.rbotlinker, 'agents should be the same')
})
